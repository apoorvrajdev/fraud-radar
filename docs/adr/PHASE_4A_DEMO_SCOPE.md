...
| Per-row deep links to `/transactions/:id` | Work for **every** listed row. *(Amended in 5G: originally a curated subset, with the rest showing a "not in snapshot" notice. That notice was unreachable in production — Vercel's SPA rewrite returns index.html with a 200 for a missing snapshot file, so the client got HTML where it expected JSON. The client now detects that, and the export covers every row so the situation is an edge case rather than the common path.)* |
...
| `transactions/{id}.json` | `GET /api/v1/transactions/{id}` | one file per listed transaction and per alert row (amended in 5G; was ~30 curated IDs) |
...
| Detail-page coverage | Every listed row *(amended in 5G; was ~30 curated IDs)* | Clicking a row is the primary interaction in the transactions view; a curated subset meant nine clicks in ten dead-ended, which reads as broken rather than bounded. At ~4 KB a page the full set is ~1.3 MB of static JSON — not a real cost on a CDN. Pinned by `backend/tests/unit/test_demo_snapshot.py`. |
