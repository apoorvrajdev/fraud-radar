...
- **`trans_date_trans_time` is the authoritative clock; `unix_time` is advisory.** The generator samples an hour of day from a shopping daypart, builds a naive `datetime`, and derives the epoch from it with `.timestamp()` ([`profile_weights.py`](https://github.com/namebrandon/Sparkov_Data_Generation/blob/master/profile_weights.py)), which resolves a naive datetime against the local timezone of whichever machine produced the corpus. The wall clock is therefore the quantity the simulation modelled — every temporal feature reads it — while the epoch carries that machine's timezone offset and nothing else. A uniform offset between the two is a provenance fact, not a data defect, so the quality report states the modal offset and how many rows sit at it.
