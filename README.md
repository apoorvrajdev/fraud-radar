...
Their results will be laid out in a generated ULB benchmark card, the next step. They are never set beside the synthetic or Sparkov results, and nothing below reports them.
...
│   │   ├── tracks/ulb/               # ULB real-data track: loader, quality report, ulb_pca_v1 matrix, trainer, verifier
...
- [ ] **5E** — Real-world benchmark track (ULB), never promoted to the serving API, with the method frozen in [`docs/adr/PHASE_5E_ULB_BENCHMARK_METHODOLOGY.md`](docs/adr/PHASE_5E_ULB_BENCHMARK_METHODOLOGY.md) before the data was acquired; steps M5A–M5G of its implementation sequence
  ...
  - [ ] **M5F** — Generated ULB benchmark card from the committed run records
  - [ ] **M5G** — What execution observed and its interpretation; README and architecture updates
...
These are the **in-house generator** numbers — the development baseline described above, not an external benchmark. The Sparkov results, the cross-generator transfer, the monthly drift series and the rules audit are reported separately in [`backend/ml/BENCHMARK_CARD.md`](backend/ml/BENCHMARK_CARD.md).
